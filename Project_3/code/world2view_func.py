...
def world2view(pts: np.ndarray, R: np.ndarray, c0: np.ndarray) -> np.ndarray:
    ...
    Parameters:
        pts (np.ndarray): 3xN matrix of 3D points in world coordinates
        R (np.ndarray): 3x3 rotation matrix defining camera orientation
        c0 (np.ndarray): 3D vector defining camera position in world coordinates
    
    Returns:
        np.ndarray: Nx3 matrix of transformed points in camera coordinates
